...
def loadLinesAndConversations(fileName):
    ...
    with open(fileName, 'r', encoding = "iso-8859-1") as f:
        for line in f:
            ...
            #Extract fields for converstation object 
...
datafile = os.path.join(corpus, "formatted-movie_lines.txt")
delimiter = '\t'
...
class Voc:
    ...
    #Remove words below a certain cout threshold
...
MAX_LENGTH = 10 #Maximum length to consider
...
def filterPair(p):
    #Input squences need to preserve the last word for EOS token
...
class LuongAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_step, last_hidden, encodder_outputs):
        ...
        attn_weights = self.attn(rnn_output, encodder_outputs)
        #Multiply attention weights to encoder outputs to get new "weighted sum" context vector
        context = attn_weights.bmm(encodder_outputs.transpose(0,1))
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding, encoder_optimizer, decoder_optimizer, batch_size, clip, max_length=MAX_LENGTH):
    ...
    #Lenghts for RNN packing shoud always be on the CPU
...
def evaluate(encoder, decoder, searcher, voc, sentence, max_length=MAX_LENGTH):
    ...
    #indexes -> words
    decoded_words = [voc.index2word[token.items()] for token in tokens]
...
#Set dropout layers to "eval" mod 
encoder.eval()
decoder.eval()
...
#Following line allows you to begin chatting
